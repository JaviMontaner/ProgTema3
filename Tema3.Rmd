...
f <- read_csv("owid-covid-data.csv")
```

Seleccionamos los países que queremos
```{r}
s <- f[f$location == 'United States' | f$location == 'Italy' | f$location == 'France' | f$location == 'Germany' | f$location == 'Canada' | f$location == 'Japan' | f$location == 'United Kingdom' | f$location == 'Spain', ]
```
...
g <- ggplot(s, aes(x = date, y = new_deaths_smoothed_per_million, colour = location)) + geom_line()
g
```
Guardamos el grafico
```{r}
ggsave(filename = 'Visualización de datos.pdf')
```
